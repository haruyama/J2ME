...
32768byte(0.0.21から, 0.0.20までは65536byte)
を超えるファイルは分割して読み込みます。
...
・ファイルの行単位で処理しておりかつ
  一度に読み込むのが32768byteのため,
  一行が15000文字以上ある文書を読み込もうとすると
  問題が発生します。(おそらく普通の文書では起こらないと思いますが)

・Shift_JISの「2バイト目に0x5Cを持つ文字（いわゆる「ダメ文字」）」
  の処理が不十分なJavaVMを利用していると 「ダメ文字」を含むフォルダや
  ファイルを読み込みに失敗します.
