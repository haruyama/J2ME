...
/**
 * XParser is a very simple lightweight XML parser. It may be used as a pull
 * parser by iterating through the element and text sections of an XML stream or
 * it may be used to read an entire XML tree into memory as XElems.
 * <p>
 * XParser works in conjunction with XWriter to support plain text or PKZIP
 * documents. This check happens automatically by sniffing the first few bytes
 * of the input stream. If a zip file is detected, the first zip entry is
 * parsed. Note that when reading from a zip file, no guarantee is made where
...
public class XParser {
     ...
     */
    /*
     * public static XParser make(File file) throws Exception { return make(new
    ...
    public static XParser make(String xml) throws Exception {
        return make(new ByteArrayInputStream(xml.getBytes("UTF-8")));
    }
     ...
     */
    /*
     * public String getEncoding() throws IOException { return in.getEncoding(); }
     */
    /**
     * Return if the stream was zipped.
     */
    /*
     * public boolean isZipped() throws IOException { return in.isZipped(); }
     */
    ...
    /**
     * Parse the entire next element into memory as a tree of XElems and
     * optionally close the underlying input stream.
     */
    ...
    /**
     * Parse the entire current element into memory as a tree of XElems and
     * optionally close the underlying input stream.
     */
    ...
    /**
     * Advance the parser to the next node and return the node type. Return the
     * current node type: ELEM_START, ELEM_END, or TEXT. If no more data to
     * parse then return EOF.
     */
    ...
    /**
     * Skip parses all the content until reaching the end tag of the specified
     * depth. When this method returns, the next call to <code>next()</code>
     * will return the element or text immediately following the end tag.
     */
    ...
    /**
     * Get the current node type constant which is always the result of the last
     * call to next(). This constant may be ELEM_START, ELEM_END, TEXT, or EOF.
     */
    ...
    /**
     * Get the depth of the current element with the document root being a depth
     * of one. A depth of 0 indicates a position before or after the root
     * element.
     */
     ...
     * element of the current character data. After ELEM_END this XElem instance
     * is no longer valid and will be reused for further processing. If depth is
     * zero return null.
     */
     ...
     * <code>depth()</code> inclusively. Calling <code>elem(0)</code> will
     * return the root element and <code>elem(depth()-1)</code> returns the
     * current element. If depth is invalid, return null.
     */
    ...
    /**
     * If the current type is TEXT return the XText instance used to store the
     * character data. After a call to <code>next()</code> this XText instance
     * is no longer valid and will be reused for further processing. If the
     * current type is not TEXT then return null.
     */
    ...
    /**
     * Parse a [40] element start production. We are passed the first character
     * after the < (beginning of name).
     */
    private void parseElemStart(int c) throws IOException, EOFException {
        ...
        while (true) {
            ...
            if (c == '>') {
                ...
                if (c != '>')
                    throw new IllegalArgumentException(
                            "Expecting /> empty element");
                emptyElem = true;
                break;
            } else {
                if (!sp)
                    throw new IllegalArgumentException(
                            "Expecting space before attribute");
    ...
    /**
     * Parse an element end production. Next character should be first char of
     * element name.
     */
    ...
    /**
     * Parse a [41] attribute production. We are passed the first character of
     * the attribute name. Return if the attribute had a namespace prefix.
     */
    private boolean parseAttr(int c, XElem elem) throws IOException,
            EOFException {
        // prefix / name
        parseQName(c);
        String prefix = this.prefix;
    ...
    /**
     * Parse an element or attribute name of the format [<prefix>:]name and
     * store result in prefix and name fields.
     */
    ...
    /**
     * Parse a character data text section. Return false if all the text was
     * whitespace only.
     */
    ...
    /**
     * Skip [3] Space = ' ' '\n' '\r' '\t' Return true if one or more space
     * chars found.
     */
     ...
     * Skip [15] Comment := <!-- ... -->
     */
    private void skipComment() throws IllegalArgumentException, IOException,
            EOFException {
        int c2 = -1, c1 = -1, c0 = -1;
        while (true) {
            c2 = c1;
            ...
            if (c2 == '-' && c1 == '-') {
                if (c0 != '>')
                    throw new IllegalArgumentException(
                            "Cannot have -- in middle of comment");
                return;
            }
        }
    }
    ...
    /**
     * Read from the stream and verify that the next characters match the
     * specified String.
     */
    private void consume(String s) throws IllegalArgumentException,
    ...
    /**
     * Read the next character from the stream: - handle pushbacks - updates the
     * line and col count - normalizes line breaks - throw EOFException if end
     * of stream reached
     */
    ...
    /**
     * Read the specified char is the amp (&) then resolve the entity otherwise
     * just return the char. If the character is markup then throw an exception.
     */
    private int toCharData(int c) throws IllegalArgumentException, IOException,
            EOFException {
        if (c == '<')
    ...
    /**
     * Map the prefix string to a XNs instance declared in the current element
     * or ancestor element.
     */
    private XNs prefixToNs(String prefix) {
        ...
        throw new IllegalArgumentException("Undeclared namespace prefix '"
                + prefix + "'");
    }
    ...
    /**
     * Recalculate what the default namespace should be because we just popped
     * the element that declared the default namespace last.
     */
    ...
    /**
     * Push a new XElem on the stack. The stack itself only allocates a new
     * XElem the first time a given depth is reached. Further pushes at that
     * depth will always reuse the last XElem from the given depth.
     */
    private XElem push() {
    ...
    /**
     * Pop decreases the element depth, but leaves the actual element in the
     * stack for reuse. However we do need to re-evaluate our namespace scope if
    ...
    /*
     * public static long mem() { return Runtime.getRuntime().totalMemory() -
     * Runtime.getRuntime().freeMemory(); }
     * 
     * public static void main(String[] args) throws Exception { long t1 =
     * System.currentTimeMillis(); System.out.println("start mem " +
     * (mem()/1024) + "kb"); XElem e = XParser.make(new File(args[0])).parse();
     * long t2 = System.currentTimeMillis(); System.out.println("finish mem " +
