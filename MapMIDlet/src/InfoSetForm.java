...
public class InfoSetForm extends Form implements CommandListener {
    ...
    private InfoSetForm() {
        super("ズーム・座標の設定");
        zoomGauge = new Gauge("ズーム", true, MapCanvas.GOOGLE_MAPS_MAX_ZOOM, 0);
        zoomGauge.setLayout(Item.LAYOUT_EXPAND);

        zoomItem = new StringItem("zoom(0,最大..." + MapCanvas.GOOGLE_MAPS_MAX_ZOOM + "最小)=", "");
        xField = new TextField("x=", "", 6, TextField.NUMERIC);
        yField = new TextField("y=", "", 6, TextField.NUMERIC);

        append(zoomGauge);
        append(zoomItem);
        append(new StringItem("マイナスのzoomは設定できません", ""));
        append(xField);
        append(yField);
