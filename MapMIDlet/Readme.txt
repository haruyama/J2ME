...
・開発履歴
0.0.35 2007/05/19 画像URLの変更に対応(v=w2.46) 
       v=w2.43のままでも大丈夫の模様
0.0.34 画像URLの変更に対応(v=w2.43)
0.0.33 画像URLの変更に対応
0.0.32 zoomの最大・最小が反映されていなかったクラスを修正
0.0.31 Google Maps の 画像URLが変更されていたのに対処
	   zoomの最小値(最大にzoomした値)を 0 から -2 に
  	   zoomの最大値(最小にzoomした値)を 0 から 17 に
	   (テストはかなり適当)
       なお, 画像自体がかなり変わっているので, 
       あたらしく取得したファイルとキャッシュを繋げて表示すると
       かなり違和感があります.
